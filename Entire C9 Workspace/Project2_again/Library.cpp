...
int searchUserFunc(User users[], int storageMax, string target) //string arr[] is array to be searched, storageMax is arr size, string target is the string to be found
{
    ...
    if (users[0].getUsername() != "") //if the user has a username set
    {
...
bool isGoodRatings(User user, int numRatings) //as long as there is at least one non-zero rating, this function will return true, and if there aren't any ratings, it returns false.
{
...
int Library::readBooks(string fileName) //GETTER - Takes string (filename) & populates books array. Returns total # of books in books array as int
{
    ...
    if (numBooks != 0)
    {
        countingVariable += numBooks; //adjusted variable if books already exist 
    }
    string line = ""; //empty string for the getline to store in.
    
    if (numBooks >= 50) //moved out to put higher priority than -1. return for if Books array gets filled
    {
        return -2;
    }
    if (myfile.is_open())
    {
        while (getline(myfile, line))
        {
            if (line.length() != 0)
            {
                string str[line.length()]; //if the line isn't empty, the array of string str is set to the length of the line.
                
                if (numBooks < sizeBook) //if the array hasn't been filled yet
                {
                    split(line, ',', str, sizeBook); //split the line
                    
                    book = Book(); //create a new book class object
                    
                    book.setAuthor(str[0]); //sets the author of the book to the first split of the line
                    
                    book.setTitle(str[1]); //sets the title to the second split
                    
                    books[countingVariable] = book; //assigns the book object to an index in the books arrray.
...
void Library::printAllBooks()
{
    ...
    if (numBooks <= 0) //if no books in array, cout statement
    {
       cout << "No books are stored" << endl;
    }
    else //otherwise, list all books.
    {
